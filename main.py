# Codigo Principal Del Boot De OTHSoporte
...
from searchandupdateot import (searchOTHControl,searchOTHControlLimitante)
...
async def main():   
    ...
    # CONSTANTS
    INPUT_DIRECTORY = 'C:\OTHSoporte\Insumos'
    INPUT_FILENAME = 'Input BOT 001 V1.xlsx'
    SUPER_LOG_FILENAME = 'C:/OTHSoporte/logs/super_log.txt'
    ...
    try:
        ...
        print("----------------- BOT ACTUALIZACION COMENTARIOS OTH CONTROL DE CAMBIOS --------------------")                        
        print("Ingresa el valor de 1 o 2 para inciar una de las opciones y cualquier otro valor para salir")
        actividad_rpa = input("1. inciar actualizacion estandar OTH control de cambios\n2. inciar actualizacion con analitica de fecha OTH control de cambiosd\n2. No se realizara actividad\n")
        if actividad_rpa == "1":
            actividad_rpa_selected = 'Procese actualizacion estandar OTH control de cambios'
        elif  actividad_rpa == "2":
            actividad_rpa_selected = 'Procese actualizacion con analitica de fecha OTH control de cambios'    
        else:
        ...
        for row in rows:
            ...
            if actividad_rpa.lower() == "1":   
                print('Validando registro: ',str(row[0].value),row[3].value)                
                resp_funcion = searchOTHControl(str(row[0].value),row[3].value)
                print("Respuesta de la función:",resp_funcion)     
            if actividad_rpa.lower() == "2":   
                print('Validando registro: ',str(row[0].value),row[3].value)                
                resp_funcion = searchOTHControlLimitante(str(row[0].value),row[3].value)
                print("Respuesta de la función:",resp_funcion)     
